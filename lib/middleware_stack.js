...
MiddlewareStack.prototype.dispatch = function dispatch (url, context, done) {
  ...
  defaultValue(context, '_parsedUrl', Url.parse(url));
  defaultValue(context, '_originalUrl', url);
  ...
  defaultValue(context, 'willBeHandled', function () {
    return context.willBeHandledOnClient() ||
      context.willBeHandledOnServer();
  });
  ...
  (function next (err) {
    ...
    if (!handler.test(url, {method: context.method}))
      return next(err);
    ...
    // Put params on the context. Each handler might add different
    // params.
    context.request.params = defaultValue(context, 'params', {});
    _.extend(context.params, handler.params(url));
  ...
  if (where === 'client' && context.willBeHandledOnServer()) {
    if (self._onServerDispatch) {
      self._onServerDispatch.call(url, context, done);
    }
  }
