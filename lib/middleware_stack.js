...
MiddlewareStack.prototype.dispatch = function (url, options) {
  options = options || {};
  ...
  thisArg.originalUrl = thisArg.originalUrl || url;
  ...
  var out = function () {
    ...
    if (where === 'client' && handlersForEnvironment.server === true) {
      if (self._onServerDispatch)
        return self._onServerDispatch.call(thisArg, url, options);
    }
  ...
  (function next (err) {
    ...
    try {
      ...
      // XXX this is a terrible api so try to clean it up
