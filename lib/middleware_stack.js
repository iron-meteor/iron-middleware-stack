...
MiddlewareStack.prototype.dispatch = function dispatch (url, context, done) {
  ...
  var next = Meteor.bindEnvironment(function boundNext (err) {
    ...
    for (self._index = self._index+1; self._index < self._stack.length; self._index++) {
      handler = self._stack[self._index];
      ...
      if (handler && handler.test(url, {method: context._method})) {
        ...
        // console.log(index, handler.where, where);
    ...
    // console.log(handler ? handler.path : 'none', url, context.url);
    ...
    try {
      ...
      console.error('Error when running specific handler', { name: handler.name, path: handler._path });
      console.error(err);
      if (err._punt)
